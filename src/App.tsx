...
function App() {
  ...
  return (
    <Box position="relative" minH="100vh" overflow="hidden" bgImage={`url(${layoutImage})`} bgSize="contain" bgRepeat="no-repeat">
      ...
      <Box position="relative" h="100vh" pointerEvents="none">{sidebarOpen ? <Box data-sidebar="decor-browser" position="absolute" top={0} right={0} bottom={0} pointerEvents="auto" zIndex={2}><DecorBrowser onClose={() => setSidebarOpen(false)} selectedDecorationName={selectedDecoration?.name ?? null} onSelectDecoration={setSelectedDecoration} /></Box> : <Box position="absolute" top={4} right={4} zIndex={4} pointerEvents="auto"><IconButton aria-label="Open" size="sm" onClick={() => setSidebarOpen(true)}><Eye /></IconButton></Box>}</Box>
    </Box>
  )
}
